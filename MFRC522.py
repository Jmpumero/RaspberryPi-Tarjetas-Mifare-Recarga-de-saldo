...
try: # Raspberry Pi
    ...
    spidev = None #work in systems with not spidev
import signal
import time
import logging
import binascii
...
class MFRC522:
    ...
    def MFRC522_DumpClassic1K(self, key, uid):
        ...
        for i in range(64):
            ...
            if status == self.MI_OK:
                block = self.MFRC522_Read(i)
                #text_read = ''.binascii.unhexlify(j).decode() for j in block)
                text_read = ''.join(chr(j) for j in block)
                #print(text_read)
                data.append(text_read)
            else:
